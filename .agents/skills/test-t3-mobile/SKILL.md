...
- App: `Arena Pair Dev`
- Bundle/package identifier: `com.t3tools.arenapair.dev`
- URL scheme: `arenapair-dev`
...
1. Inspect any process on the intended Metro port and its `/status` response. Reuse it only when it is healthy, belongs to this worktree, and matches `APP_VARIANT=development`, `--dev-client`, and scheme `arenapair-dev`.
...
- Workspace: `<repo>/apps/mobile/ios/ArenaPairDev.xcworkspace`
- Scheme: `ArenaPairDev`
- Configuration: `Debug`
- Simulator ID: the selected UDID
- Bundle ID: `com.t3tools.arenapair.dev`

Check the installed client with:

```bash
xcrun simctl get_app_container <simulator-udid> com.t3tools.arenapair.dev app
...
```bash
adb -s <emulator-serial> shell pm path com.t3tools.arenapair.dev
...
```text
arenapair-dev://connections/new?pairingUrl=<encoded-pairing-url>&autoConnect=1
```
...
Use `snapshot_ui` and current element references from XcodeBuildMCP for taps and typing. Stream the same UDID through `ios-simulator-browser` so the user can watch in Arena Pair when the host supports it. Use the stream as a visual feed rather than a reason to switch to fragile browser coordinates.
...
3. Remove the disposable environment from Arena Pair Dev.
