---
name: test-t3-app
description: Launch, retain, and test the Arena Pair web app in isolated development environments, including first-try browser authentication with one-time pairing URLs, pairing-token recovery, worktree-safe state directories, cross-turn dev server lifecycle, and direct SQLite inspection or fixture seeding. Use when an agent needs to run T3 locally, iteratively test UI behavior with a human, recover from an expired or consumed pairing token, isolate dev state, or prepare test data in state.sqlite.
---
...
2. Choose a base directory that belongs only to the current worktree or test:
   ...
   - Use `mktemp -d /tmp/arenapair-test.XXXXXX` for disposable state and retain the printed absolute path.
...
The worktree-local default deliberately outranks an ambient `ARENAPAIR_HOME`; do not pass the shared home through to a worktree dev server.
