...
describe("mobile connection storage", () => {
  ...
  it("preserves secure-storage read failures with operation and key context", async () => {
    ...
    await expect(loadSavedConnections()).rejects.toMatchObject({
      ...
      operation: "read",
      key: "arenapair.connections",
      cause,
      message: "Mobile secure storage operation read failed for key arenapair.connections.",
    });
  });

  it("logs structured decode failures before using the empty fallback", async () => {
    await mocks.setItemAsync("arenapair.connections", "{");
    ...
    expect(warn).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        message: "Failed to decode mobile storage value for key arenapair.connections.",
      }),
    );
  ...
  it("loads legacy preferences when SQLite is unavailable", async () => {
    ...
    await mocks.setItemAsync("arenapair.preferences", JSON.stringify({ baseFontSize: 17 }));
  ...
  it("falls back to secure storage when SQLite cannot save preferences", async () => {
    ...
    const fallback = JSON.parse(mocks.getStoredValue("arenapair.preferences.fallback") ?? "") as {
  ...
  it("reconciles fallback preferences after SQLite recovers", async () => {
    ...
    await mocks.setItemAsync(
      "arenapair.preferences.fallback",
      JSON.stringify({
    ...
    expect(mocks.getStoredValue("arenapair.preferences.fallback")).toBeNull();
  });
  ...
  it("keeps SQLite authoritative when stale legacy preferences remain", async () => {
    mocks.setPreferencesJson(JSON.stringify({ baseFontSize: 21 }), 30);
    await mocks.setItemAsync("arenapair.preferences", JSON.stringify({ baseFontSize: 19 }));
