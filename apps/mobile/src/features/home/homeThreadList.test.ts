...
describe("buildHomeThreadGroups", () => {
  it("builds one v2 scope for the same repository across environments", () => {
    ...
    const repositoryIdentity = {
      ...
      locator: {
        ...
        remoteName: "origin",
        remoteUrl: "git@github.com:pingdotgg/arenapair.git",
      },
    };
    const projects = [
      makeProject({
    ...
    expect(scopes[0]?.title).toBe("arenapair");
  ...
  it("routes stale duplicate project refs through the canonical repository group", () => {
    ...
    const local = makeProject({
      ...
      environmentId: localEnvironmentId,
      title: "arenapair",
      workspaceRoot: "/workspaces/arenapair",
      repositoryIdentity,
    });
    const stale = makeProject({
      ...
      title: "arenapair",
      workspaceRoot: "/remote/arenapair",
      updatedAt: "2026-06-01T00:00:00.000Z",
    });
  ...
  it("matches web repository, repository-path, and separate grouping modes", () => {
    ...
    const repositoryIdentity = {
      ...
      locator: {
        source: "git-remote" as const,
        remoteName: "origin",
        remoteUrl: "git@github.com:t3tools/arenapair.git",
      },
      provider: "github",
      owner: "t3tools",
      name: "arenapair",
      displayName: "Arena Pair",
      rootPath: "/workspaces/arenapair",
    };
    const projects = [
      makeProject({
        environmentId,
        ...
        title: "Web",
        workspaceRoot: "/workspaces/arenapair/apps/web",
        repositoryIdentity,
      }),
      makeProject({
        environmentId,
        ...
        title: "Mobile",
        workspaceRoot: "/workspaces/arenapair/apps/mobile",
        repositoryIdentity,
      }),
    ];
    const threads = projects.map((project) =>
  ...
  it("does not apply the recency window while searching", () => {
    ...
    const group = buildGroups([project], threads, { searchQuery: "Arena Pair" })[0];
