  ...
  it("removes a Clerk virtual pathname and callback params while preserving the desktop route", () => {
    expect(
      resolveClerkSignInProps(
        "arenapair://app/CLERK-ROUTER/VIRTUAL/sign-up?__clerk_status=complete#/settings/connections",
        true,
      ),
    ).toEqual({
      forceRedirectUrl: "arenapair://app/#/settings/connections",
      signUpForceRedirectUrl: "arenapair://app/#/settings/connections",
    });
  });

  it("preserves a clean development desktop route", () => {
    expect(resolveClerkSignInProps("arenapair-dev://app/#/settings/general", true)).toEqual({
      forceRedirectUrl: "arenapair-dev://app/#/settings/general",
      signUpForceRedirectUrl: "arenapair-dev://app/#/settings/general",
    });
  });
});
