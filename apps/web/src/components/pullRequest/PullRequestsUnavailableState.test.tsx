...
describe("PullRequestsUnavailableState", () => {
  it("can explain an unsupported environment without offering a futile retry", () => {
    ...
    expect(text).toContain("Update this environment's Arena Pair server");
  ...
  it("retains the retry for transient load failures", () => {
    const html = renderToStaticMarkup(
      <PullRequestsUnavailableState
        ...
        onRetry={() => {}}
        gitHubUrl="https://github.com/pingdotgg/arenapair/pull/42"
      />,
    );

    expect(html).toContain("Retry");
    expect(html).toContain("Open on GitHub");
    expect(html).toContain('href="https://github.com/pingdotgg/arenapair/pull/42"');
  ...
  it("can offer the browser without offering a retry", () => {
    const html = renderToStaticMarkup(
      <PullRequestsUnavailableState
        ...
        gitHubUrl="https://github.com/pingdotgg/arenapair/pull/9"
      />,
    );
