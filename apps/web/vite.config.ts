...
const isSingleOriginDev = process.env.ARENAPAIR_SINGLE_ORIGIN_DEV === "1";
...
const configuredRelayUrl = repoEnv.VITE_ARENAPAIR_RELAY_URL?.trim() || "";
...
const sourcemapEnv = process.env.ARENAPAIR_WEB_SOURCEMAP?.trim().toLowerCase();
...
// as hot patches. Opt-in while experimental: ARENAPAIR_BUNDLED_DEV=1 pnpm dev:web
// The dev runner defaults this on for --share runs (remote browsers pay a
// round trip per import level in unbundled dev); ARENAPAIR_BUNDLED_DEV=0 opts out.
const bundledDevEnv = process.env.ARENAPAIR_BUNDLED_DEV?.trim().toLowerCase();
...
function resolveDevProxyTarget(
  ...
  // ARENAPAIR_PORT is set by scripts/dev-runner.ts for every non-desktop mode.
...
const devProxyTarget = resolveDevProxyTarget(process.env.ARENAPAIR_PORT, configuredWsUrl);
...
function devCompressionPlugin(): Plugin {
  return {
    name: "arenapair:dev-compression",
    apply: "serve",
...
const configuredAllowedHosts = (process.env.ARENAPAIR_DEV_ALLOWED_HOSTS ?? "")
  .split(",")
...
export default defineConfig(() => {
  return {
    ...
    define: {
      ...
      "import.meta.env.VITE_ARENAPAIR_RELAY_URL": JSON.stringify(configuredRelayUrl),
