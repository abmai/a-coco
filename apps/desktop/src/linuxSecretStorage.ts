...
function getGnomeKeyringRemediationMessage(): string {
  return "Arena Pair could not access GNOME Keyring to save this environment credential. Install and start GNOME Keyring, then restart Arena Pair.";
}

function getKWalletRemediationMessage(): string {
  return "Arena Pair could not access KWallet to save this environment credential. Enable the KDE wallet subsystem in System Settings, then restart Arena Pair.";
}
