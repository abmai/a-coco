// This file mostly exists because we want dev mode to say "Arena Pair (Dev)" instead of "electron"
...
const devBundleIdSuffix = NodePath.basename(repoRoot)
  .toLowerCase()
  .replaceAll(/[^a-z0-9]+/g, "");
export const APP_DISPLAY_NAME = isDevelopment ? "Arena Pair (Dev)" : "Arena Pair (Alpha)";
export const APP_BUNDLE_ID = isDevelopment
  ? `com.t3tools.arenapair.dev.${devBundleIdSuffix || "local"}`
  : "com.t3tools.arenapair";
const APP_PROTOCOL_SCHEMES = isDevelopment ? ["arenapair-dev"] : ["arenapair"];
...
// oxlint-disable-next-line arenapair/no-global-process-runtime -- Standalone launcher script has no Effect runtime.
...
export function makeDevelopmentLauncherScript({
  ...
  const envEntries = [
    ...
    ["ARENAPAIR_PORT", environment.ARENAPAIR_PORT],
    ["ARENAPAIR_HOME", environment.ARENAPAIR_HOME],
    ["ARENAPAIR_COMMIT_HASH", environment.ARENAPAIR_COMMIT_HASH],
    ["ARENAPAIR_OTLP_TRACES_URL", environment.ARENAPAIR_OTLP_TRACES_URL],
    ["ARENAPAIR_OTLP_EXPORT_INTERVAL_MS", environment.ARENAPAIR_OTLP_EXPORT_INTERVAL_MS],
    ["ARENAPAIR_DESKTOP_APP_USER_MODEL_ID", APP_BUNDLE_ID],
  ...
  return [
    ...
    `exec ${shellSingleQuote(electronBinaryPath)} --arenapair-dev-root=${shellSingleQuote(desktopRoot)} ${shellSingleQuote(mainEntryPath)} "$@"`,
    "",
  ].join("\n");
}
...
function buildMacLauncher(electronBinaryPath) {
  ...
  if (isDevelopment) {
    ...
    // Electron.app even though this bundle's Info.plist has the Arena Pair name.
