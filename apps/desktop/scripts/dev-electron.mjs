...
const remoteDebuggingPort = process.env.ARENAPAIR_DESKTOP_REMOTE_DEBUGGING_PORT?.trim();
// oxlint-disable-next-line arenapair/no-global-process-runtime -- Standalone dev script has no Effect runtime.
...
if (devProtocolClient) {
  childEnv.ARENAPAIR_DESKTOP_APP_USER_MODEL_ID = devProtocolClient.appBundleId;
  childEnv.ARENAPAIR_DESKTOP_PROTOCOL_REGISTRATION_MANAGED = "1";
}
...
function cleanupStaleDevApps() {
  ...
  NodeChildProcess.spawnSync("pkill", ["-f", "--", `--arenapair-dev-root=${desktopDir}`], {
    stdio: "ignore",
  });
}
...
function startApp() {
  ...
  const launchArgs = devProtocolClient
    ? electronArgs
    : [...electronArgs, `--arenapair-dev-root=${desktopDir}`, "dist-electron/main.cjs"];
