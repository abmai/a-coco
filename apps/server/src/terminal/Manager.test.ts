...
> =>
  Effect.flatMap(Effect.service(FileSystem.FileSystem), (fs) =>
    Effect.gen(function* () {
      ...
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "arenapair-terminal-" });
...
it.layer(
  ...
  it.effect("attaches to exited sessions without restarting them", () =>
    Effect.gen(function* () {
      ...
      const unsubscribe = yield* manager.attachStream(
        openInput({
          env: {
            ARENAPAIR_WORKTREE_PATH: "/tmp/should-not-restart",
          },
      ...
      const unsubscribe = yield* manager.attachStream(
        {
          ...openInput({
            env: {
              ARENAPAIR_WORKTREE_PATH: "/tmp/restart-requested",
            },
  ...
  it.effect("filters app runtime env variables from terminal sessions", () =>
    Effect.gen(function* () {
      ...
      expect(spawnInput.env.ARENAPAIR_PORT).toBeUndefined();
  ...
  it.effect("strips AppImage runtime env from terminal sessions", () =>
    Effect.gen(function* () {
      const appDir = "/tmp/.mount_ArenaPairabc123";
      const { manager, ptyAdapter } = yield* createManager(5, {
        env: {
  ...
  it.effect("injects runtime env overrides into spawned terminals", () =>
    Effect.gen(function* () {
      ...
      yield* manager.open(
        openInput({
          env: {
            ARENAPAIR_PROJECT_ROOT: "/repo",
            ARENAPAIR_WORKTREE_PATH: "/repo/worktree-a",
            CUSTOM_FLAG: "1",
          },
        }),
      );
      ...
      assert.equal(spawnInput.env.ARENAPAIR_PROJECT_ROOT, "/repo");
      assert.equal(spawnInput.env.ARENAPAIR_WORKTREE_PATH, "/repo/worktree-a");
