...
export const TRIAGE_PLAYBOOK = `# Arena Pair triage playbook

You are a support engineer for Arena Pair (https://github.com/pingdotgg/arenapair), working
...
Fetch https://raw.githubusercontent.com/pingdotgg/arenapair/main/.github/triage/PLAYBOOK.md.
...
cache directory named in the context file, one subdirectory per commit hash:
    ...
    git clone --depth 1 --filter=blob:none --branch <release-tag> \\
      https://github.com/pingdotgg/arenapair <source-cache-dir>/<hash>
...
- How is Arena Pair running on this machine: \`npx t3 serve\` in a terminal, the
...
Search existing issues in pingdotgg/arenapair (use \`gh\`, or the public GitHub search
...
Do not patch the Arena Pair source as a fix. A good issue with strong repro steps
...
- If \`gh\` is not authenticated, offer \`gh auth login\`, or build a prefilled
  https://github.com/pingdotgg/arenapair/issues/new URL with title and body query
...
export const buildTriageLaunchPrompt = (promptFilePath: string) =>
  `Read the file "${promptFilePath}" and follow its instructions exactly: it is your Arena Pair triage playbook, and it starts with asking the user what went wrong.`;

/** The full seed prompt, written to `prompt.md` in the triage scratch dir. */
export const buildTriageSeedPrompt = (contextFilePath: string) => `A Arena Pair user is \
...
export const buildTriageContext = (input: TriageContextInput) => `# Arena Pair triage context
...
- Repo: https://github.com/pingdotgg/arenapair
