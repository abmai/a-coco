...
repo='pingdotgg/arenapair'
tag="${RELEASE_TAG:?RELEASE_TAG is required}"
pkgrel="${PKGREL:-1}"

if [[ "$tag" =~ ^v[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
  pkgname='arenapair-bin'
elif [[ "$tag" =~ ^v[0-9]+\.[0-9]+\.[0-9]+-nightly\.[0-9]{8}\.[0-9]+$ ]]; then
  pkgname='arenapair-nightly-bin'
else
...
version="${tag#v}"
pkgver="${version//-/_}"
asset_name="Arena-Pair-${version}-x86_64.AppImage"
...
cd "$aur_dir"
git rm --ignore-unmatch LICENSE .upstream-commit arenapair-icon.png
git config user.name 'arenapair-ci'
git config user.email 'arenapair-ci@users.noreply.github.com'
git add -A
