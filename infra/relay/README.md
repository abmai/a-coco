...
The relay is intentionally not in the hot path for normal Arena Pair traffic. After a client connects,
...
- Linking Arena Pair environments to a cloud account.
...
```sh
vp run --filter arenapair-relay deploy
```
...
```sh
vp run --filter arenapair-relay deploy -- --stage prod
vp run --filter arenapair-relay deploy -- --env-file .env.local
```
