...
describe("managed endpoint names", () => {
  it("uses the stage slug and a stable stage-scoped digest suffix", () => {
    ...
    expect(managedEndpointHostname("prod", "arenapairrelay.com", hash)).toBe(
      "prod-abcdef0123456789.arenapairrelay.com",
    );
    expect(managedEndpointTunnelName("dev_julius", hash)).toBe(
      "arenapairrelay-managedendpoint-dev-julius-abcdef0123456789",
    );
  });
