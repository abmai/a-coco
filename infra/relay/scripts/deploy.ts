...
const publicConfigEnvEntries = (config: RelayPublicConfig) =>
  ({
    ...
    ARENAPAIR_MOBILE_OTLP_TRACES_URL: config.mobileTracingUrl,
    ARENAPAIR_MOBILE_OTLP_TRACES_DATASET: config.mobileTracingDataset,
    ARENAPAIR_MOBILE_OTLP_TRACES_TOKEN: config.mobileTracingToken,
    ARENAPAIR_RELAY_CLIENT_OTLP_TRACES_URL: config.clientTracingUrl,
    ARENAPAIR_RELAY_CLIENT_OTLP_TRACES_DATASET: config.clientTracingDataset,
    ARENAPAIR_RELAY_CLIENT_OTLP_TRACES_TOKEN: config.clientTracingToken,
  }) as const;
...
export function reconcileRootEnvRelayUrl(contents: string, relayUrl: string): string {
  return reconcileRootEnvPublicConfig(contents, {
    ...
    .split("\n")
    .filter((line) => !line.startsWith("ARENAPAIR_MOBILE_OTLP_TRACES_"))
    .filter((line) => !line.startsWith("ARENAPAIR_RELAY_CLIENT_OTLP_TRACES_"))
    .join("\n");
}
...
const readRelayPublicConfig = Effect.fn("relay.deploy.readState")(function* (stage: string) {
  ...
  const output = yield* service.getOutput({ stack: "ArenaPairRelay", stage });
...
export const relayDeployCommand = Command.make(
  ...
  deploy,
).pipe(Command.withDescription("Deploy the Arena Pair relay through Alchemy."));
