...
describe("reconcileRootEnvPublicConfig", () => {
  ...
  it("adds the complete local client config", () => {
    expect(reconcileRootEnvPublicConfig("", config)).toBe(
      [
        ...
        "ARENAPAIR_MOBILE_OTLP_TRACES_DATASET=t3-code-mobile-traces-dev",
        "ARENAPAIR_MOBILE_OTLP_TRACES_TOKEN=xaat-public-ingest",
        "ARENAPAIR_RELAY_CLIENT_OTLP_TRACES_URL=https://api.axiom.co/v1/traces",
        "ARENAPAIR_RELAY_CLIENT_OTLP_TRACES_DATASET=t3-code-relay-client-traces-dev",
        "ARENAPAIR_RELAY_CLIENT_OTLP_TRACES_TOKEN=xaat-relay-client-ingest",
        "",
      ].join("\n"),
    );
  });
  ...
  it("replaces stale values while preserving unrelated entries", () => {
    expect(
      reconcileRootEnvPublicConfig(
        [
          "ARENAPAIR_CLERK_PUBLISHABLE_KEY=pk_test_example",
          "ARENAPAIR_RELAY_URL=https://old.example.test",
          "ARENAPAIR_MOBILE_OTLP_TRACES_URL=https://old.example.test/v1/traces",
          "ARENAPAIR_MOBILE_OTLP_TRACES_DATASET=old-dataset",
          "ARENAPAIR_MOBILE_OTLP_TRACES_TOKEN=old-token",
          "ARENAPAIR_RELAY_CLIENT_OTLP_TRACES_URL=https://old.example.test/v1/traces",
          "ARENAPAIR_RELAY_CLIENT_OTLP_TRACES_DATASET=old-client-dataset",
          "ARENAPAIR_RELAY_CLIENT_OTLP_TRACES_TOKEN=old-client-token",
          "",
        ].join("\n"),
        config,
      ),
...
describe("serializeRelayClientTracingEnvironment", () => {
  it("serializes tracing config for downstream GITHUB_ENV loading", () => {
    expect(
      ...
      [
        "ARENAPAIR_RELAY_CLIENT_OTLP_TRACES_URL=https://api.axiom.co/v1/traces",
        "ARENAPAIR_RELAY_CLIENT_OTLP_TRACES_DATASET=relay",
        "ARENAPAIR_RELAY_CLIENT_OTLP_TRACES_TOKEN=client-token",
        "",
      ].join("\n"),
    );
  });
