...
describe("loadRepoEnv", () => {
  it("does not project cloud configuration for an unconfigured clone", () => {
    ...
    expect(env.ARENAPAIR_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.ARENAPAIR_CLERK_CLI_OAUTH_CLIENT_ID).toBeUndefined();
    expect(env.VITE_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.EXPO_PUBLIC_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.ARENAPAIR_CLERK_JWT_TEMPLATE).toBeUndefined();
    expect(env.VITE_CLERK_JWT_TEMPLATE).toBeUndefined();
    expect(env.EXPO_PUBLIC_CLERK_JWT_TEMPLATE).toBeUndefined();
    expect(env.ARENAPAIR_RELAY_URL).toBeUndefined();
    expect(env.VITE_ARENAPAIR_RELAY_URL).toBeUndefined();
    expect(env.ARENAPAIR_MOBILE_OTLP_TRACES_URL).toBeUndefined();
    expect(env.ARENAPAIR_MOBILE_OTLP_TRACES_DATASET).toBeUndefined();
    expect(env.ARENAPAIR_MOBILE_OTLP_TRACES_TOKEN).toBeUndefined();
    ...
    expect(env.ARENAPAIR_RELAY_CLIENT_OTLP_TRACES_URL).toBeUndefined();
    expect(env.ARENAPAIR_RELAY_CLIENT_OTLP_TRACES_DATASET).toBeUndefined();
    expect(env.ARENAPAIR_RELAY_CLIENT_OTLP_TRACES_TOKEN).toBeUndefined();
  ...
  it("applies process, root local, and root precedence in that order", () => {
    ...
    NodeFS.writeFileSync(
      ...
      "ARENAPAIR_CLERK_PUBLISHABLE_KEY=pk_root\nARENAPAIR_CLERK_JWT_TEMPLATE=template_root\nARENAPAIR_CLERK_CLI_OAUTH_CLIENT_ID=oauth_root\nARENAPAIR_RELAY_URL=https://root.example.test\n",
    );
    NodeFS.writeFileSync(
      NodePath.join(repoRoot, ".env.local"),
      "ARENAPAIR_CLERK_PUBLISHABLE_KEY=pk_local\nARENAPAIR_CLERK_JWT_TEMPLATE=template_local\nARENAPAIR_CLERK_CLI_OAUTH_CLIENT_ID=oauth_local\nARENAPAIR_RELAY_URL=https://local.example.test\n",
    );

    expect(loadRepoEnv({ baseEnv: {}, repoRoot }).ARENAPAIR_RELAY_URL).toBe(
      "https://local.example.test",
    );
    expect(
      loadRepoEnv({
        baseEnv: {
          ARENAPAIR_CLERK_PUBLISHABLE_KEY: "pk_ci",
          ARENAPAIR_CLERK_JWT_TEMPLATE: "template_ci",
          ARENAPAIR_CLERK_CLI_OAUTH_CLIENT_ID: "oauth_ci",
          ARENAPAIR_RELAY_URL: "https://ci.example.test",
        },
        repoRoot,
      }),
    ).toMatchObject({
      ...
      ARENAPAIR_RELAY_URL: "https://ci.example.test",
      VITE_ARENAPAIR_RELAY_URL: "https://ci.example.test",
    });
  });
  ...
  it("accepts legacy framework aliases as root overrides", () => {
    expect(
      resolvePublicConfig({
        ...
        ARENAPAIR_CLERK_CLI_OAUTH_CLIENT_ID: "oauth_canonical",
        VITE_ARENAPAIR_RELAY_URL: "https://legacy.example.test",
  ...
  it("projects canonical relay client tracing values to web build aliases", () => {
    expect(
      loadRepoEnv({
        baseEnv: {
          ARENAPAIR_RELAY_CLIENT_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
          ARENAPAIR_RELAY_CLIENT_OTLP_TRACES_DATASET: "relay-client-traces",
          ARENAPAIR_RELAY_CLIENT_OTLP_TRACES_TOKEN: "relay-client-token",
        },
  ...
  it("projects canonical mobile tracing values to Expo public aliases", () => {
    expect(
      loadRepoEnv({
        baseEnv: {
          ARENAPAIR_RELAY_URL: "https://relay.example.test",
          ARENAPAIR_MOBILE_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
          ARENAPAIR_MOBILE_OTLP_TRACES_DATASET: "mobile-traces",
          ARENAPAIR_MOBILE_OTLP_TRACES_TOKEN: "mobile-token",
        },
        repoRoot: makeTemporaryDirectory(),
      }),
    ).toEqual({
      ARENAPAIR_RELAY_URL: "https://relay.example.test",
      VITE_ARENAPAIR_RELAY_URL: "https://relay.example.test",
      ARENAPAIR_MOBILE_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
      ARENAPAIR_MOBILE_OTLP_TRACES_DATASET: "mobile-traces",
      ARENAPAIR_MOBILE_OTLP_TRACES_TOKEN: "mobile-token",
      EXPO_PUBLIC_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
...
function makeTemporaryDirectory() {
  const directory = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "arenapair-public-config-"));
