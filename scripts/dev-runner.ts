...
/**
 * `role` matters because only the backend honours `--host`/`ARENAPAIR_HOST`; the
...
export class DevRunnerInvalidPortOffsetError extends Schema.TaggedErrorClass<DevRunnerInvalidPortOffsetError>()(
  ...
  {
    configKey: Schema.Literal("ARENAPAIR_PORT_OFFSET"),
...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("ARENAPAIR_PORT_OFFSET"),
  devInstance: optionalStringConfig("ARENAPAIR_DEV_INSTANCE"),
});
...
> {
  if (config.portOffset !== undefined) {
    if (config.portOffset < 0) {
      return Effect.fail(
        new DevRunnerInvalidPortOffsetError({
          configKey: "ARENAPAIR_PORT_OFFSET",
    ...
    return Effect.succeed({
      ...
      source: `ARENAPAIR_PORT_OFFSET=${config.portOffset}`,
    });
  }
  ...
  if (seed) {
    if (/^\d+$/.test(seed)) {
      return Effect.succeed({
        offset: Number(seed),
        source: `numeric ARENAPAIR_DEV_INSTANCE=${seed}`,
      });
    }

    const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
    return Effect.succeed({ offset, source: `hashed ARENAPAIR_DEV_INSTANCE=${seed}` });
  }
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    // Precedence (--home-dir > worktree .t3 > ambient ARENAPAIR_HOME) is resolved
    ...
    if (configuredBaseDir !== undefined) {
      output.ARENAPAIR_HOME = resolvedBaseDir;
    } else {
      delete output.ARENAPAIR_HOME;
    }
    ...
    // agent working inside Arena Pair), these leak through and the child server
    ...
    if (!isDesktopMode) {
      output.ARENAPAIR_PORT = String(serverPort);
      ...
      if (mode === "dev" || mode === "dev:web") {
        ...
        output.ARENAPAIR_SINGLE_ORIGIN_DEV = "1";
      } else {
        output.VITE_HTTP_URL = `http://localhost:${serverPort}`;
        output.VITE_WS_URL = `ws://localhost:${serverPort}`;
        delete output.ARENAPAIR_SINGLE_ORIGIN_DEV;
      }
    } else {
      output.ARENAPAIR_PORT = String(serverPort);
      ...
      delete output.ARENAPAIR_SINGLE_ORIGIN_DEV;
      delete output.ARENAPAIR_MODE;
      delete output.ARENAPAIR_NO_BROWSER;
      delete output.ARENAPAIR_HOST;
    }

    if (!isDesktopMode && host !== undefined) {
      output.ARENAPAIR_HOST = host;
    }

    if (!isDesktopMode) {
      output.ARENAPAIR_NO_BROWSER = browser === true ? "0" : "1";
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.ARENAPAIR_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.ARENAPAIR_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.ARENAPAIR_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.ARENAPAIR_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.ARENAPAIR_MODE = "web";
      delete output.ARENAPAIR_DESKTOP_WS_URL;
    }
 ...
 * target reach it there. When `--host`/`ARENAPAIR_HOST` moves the backend onto
...
export function runDevRunnerWithInput(input: DevRunnerCliInput) {
  return Effect.gen(function* () {
      ...
      Effect.mapError(
        (cause) =>
          new DevRunnerConfigurationError({
            configKeys: ["ARENAPAIR_PORT_OFFSET", "ARENAPAIR_DEV_INSTANCE"],
            cause,
          }),
      ),
    );
    ...
    // outrank an ambient ARENAPAIR_HOME. `--home-dir` still wins.
    ...
    const resolvedT3Home =
      ...
      worktreeHome ??
      (hostEnvironment.ARENAPAIR_HOME?.trim() || undefined);
    ...
    const selectionSuffix =
      serverOffset !== offset || webOffset !== offset
        ...
        : "";
    const baseDir = env.ARENAPAIR_HOME ?? (yield* DEFAULT_T3_HOME);

    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.ARENAPAIR_PORT)} webPort=${String(env.PORT)} baseDir=${baseDir}`,
    );
    ...
    if (input.share) {
      if (input.mode === "dev:server") {
        ...
        if (shared) {
          ...
          env.ARENAPAIR_DEV_ALLOWED_ORIGINS = [
            env.ARENAPAIR_DEV_ALLOWED_ORIGINS,
          ...
          // a few chunk requests. Only defaulted, so ARENAPAIR_BUNDLED_DEV=0
          // still opts a --share run back out.
          if (env.ARENAPAIR_BUNDLED_DEV === undefined) {
            env.ARENAPAIR_BUNDLED_DEV = "1";
          }
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  t3Home: Flag.string("home-dir").pipe(
    Flag.withDescription(
      "Explicit Arena Pair data directory; runtime state is stored under userdata (equivalent to ARENAPAIR_HOME). Inside a git worktree this defaults to that worktree's own .t3 so dev state stays off the shared home.",
    ),
    Flag.optional,
  ...
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to ARENAPAIR_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("ARENAPAIR_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription("WebSocket event logging toggle (equivalent to ARENAPAIR_LOG_WS_EVENTS)."),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("ARENAPAIR_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to ARENAPAIR_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("ARENAPAIR_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to ARENAPAIR_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("ARENAPAIR_PORT")),
  ),
