...
const repositoryIdentity = {
  canonicalKey: "github.com/t3tools/arenapair",
  locator: {
    source: "git-remote" as const,
    remoteName: "upstream",
    remoteUrl: "https://github.com/t3tools/arenapair.git",
  },
  provider: "github",
  owner: "t3tools",
  name: "arenapair",
  displayName: "Arena Pair",
};
...
describe("buildProjectGroups", () => {
  it("preserves every physical clone as a selectable member in repository modes", () => {
    const projects = [
      makeProject("arenapair", "/work/arenapair"),
      makeProject("arenapair-2", "/work/arenapair-2"),
      makeProject("arenapair-3", "/work/arenapair-3"),
    ];
  ...
  it("uses a shared custom title as the repository group's label", () => {
    const projects = [
      makeProject("first", "/work/arenapair", { title: "Custom project" }),
      makeProject("second", "/work/arenapair-2", { title: "Custom project" }),
    ];
  ...
  it("keeps the repository label when shared titles match its repository name", () => {
    const projects = [
      makeProject("first", "/work/arenapair", { title: "arenapair" }),
      makeProject("second", "/work/arenapair-2", { title: "arenapair" }),
    ];
  ...
  it("keeps physical clones in separate groups when requested", () => {
    ...
    expect(groups.map((group) => group.label)).toEqual(["arenapair", "arenapair-2", "arenapair-3"]);
  });

  it("applies a physical-project override without dropping its siblings", () => {
    const first = makeProject("arenapair", "/work/arenapair");
    const second = makeProject("arenapair-2", "/work/arenapair-2");
    const third = makeProject("arenapair-3", "/work/arenapair-3");
  ...
  it("dedupes stale registrations at one physical path using the freshest project", () => {
    const stale = makeProject("stale", "/work/arenapair", {
      repositoryIdentity: null,
      updatedAt: "2026-07-01T00:00:00.000Z",
    });
    const fresh = makeProject("fresh", "/work/arenapair/", {
  ...
  it("uses repository identity from a duplicate registration when the winner lacks it", () => {
    const identified = makeProject("identified", "/work/arenapair", {
      updatedAt: "2026-07-01T00:00:00.000Z",
    });
    const freshUnidentified = makeProject("fresh", "/work/arenapair/", {
      repositoryIdentity: null,
      updatedAt: "2026-07-02T00:00:00.000Z",
    });
    const sibling = makeProject("sibling", "/work/arenapair-2");
  ...
  it("uses the freshest identity-bearing duplicate when the winner lacks identity", () => {
    ...
    const staleIdentified = makeProject("stale-identified", "/work/arenapair", {
      repositoryIdentity: staleIdentity,
      updatedAt: "2026-07-01T00:00:00.000Z",
    });
    const freshIdentified = makeProject("fresh-identified", "/work/arenapair/", {
      updatedAt: "2026-07-02T00:00:00.000Z",
    });
    const winner = makeProject("winner", "/work/arenapair", {
      repositoryIdentity: null,
