...
const SSH_ASKPASS_DIR_NAME = "arenapair-ssh-askpass";
...
# Invoked by ssh via SSH_ASKPASS when Arena Pair re-runs ssh with a cached password
...
printf 'Arena Pair ssh-askpass invoked without T3_SSH_AUTH_SECRET.\\n' >&2
exit 1
`;
...
export const ASKPASS_WINDOWS_SCRIPT = `# Invoked by ssh via SSH_ASKPASS (through ssh-askpass.cmd) when Arena Pair re-runs\r
...
[Console]::Error.WriteLine("Arena Pair ssh-askpass invoked without T3_SSH_AUTH_SECRET.")\r
exit 1\r
`;
...
export const getDefaultSshAskpassDirectory = Effect.fn("ssh/auth.getDefaultSshAskpassDirectory")(
  function* () {
    ...
    const parentDirectory = yield* fs.makeTempDirectory({ prefix: "arenapair-ssh-runtime-" });
...
> {
  ...
  return {
    ...
    ...(platform === "win32" || baseEnv.DISPLAY || hostDisplay ? {} : { DISPLAY: "arenapair" }),
  };
});
