...
export const createOxlintRuleHarness = (
  ...
  const run: RuleHarness["run"] = Effect.fnUntraced(function* (source: string) {
    ...
    const fixtureDir = yield* fs.makeTempDirectoryScoped({ prefix: "arenapair-oxlint-" });
    ...
    const pluginPath = path.join(repoRoot, "oxlint-plugin-arenapair", "index.ts");

    yield* fs.writeFileString(
      configPath,
      yield* encodeOxlintConfig({
        jsPlugins: [{ name: "arenapair", specifier: pluginPath }],
