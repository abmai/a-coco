...
- `apps/desktop` (`@t3tools/desktop`): Electron shell. Supervises a desktop-scoped `t3` backend,
  loads the web bundle over the `arenapair://` protocol, and owns SSH-managed remote environments.
...
- `infra/relay` (`arenapair-relay`): the hosted T3 Connect relay, deployed with Alchemy. Handles
...
- `oxlint-plugin-arenapair/`: repo-specific lint rules.
