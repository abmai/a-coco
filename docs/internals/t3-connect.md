...
> For maintainers. Using Arena Pair? See [docs/user](../user/).
...
```dotenv
ARENAPAIR_CLERK_PUBLISHABLE_KEY=<publishable key>
ARENAPAIR_CLERK_JWT_TEMPLATE=<JWT template name>
ARENAPAIR_CLERK_CLI_OAUTH_CLIENT_ID=<public OAuth application client ID>
ARENAPAIR_RELAY_URL=https://relay.example.com
```
...
should set `ARENAPAIR_CLERK_PUBLISHABLE_KEY`, `ARENAPAIR_CLERK_JWT_TEMPLATE`,
`ARENAPAIR_CLERK_CLI_OAUTH_CLIENT_ID`, and `ARENAPAIR_RELAY_URL` before building. EAS preview and
...
deployment defaults.
`vp run --filter arenapair-relay deploy` invokes Alchemy from the relay directory, so Alchemy loads
...
3. Add **both** allowed redirect URIs:
   ...
   - `https://app.t3.codes/connect/callback` for the hosted out-of-band flow. This is
     ...
     custom `ARENAPAIR_HOSTED_APP_URL` means `$ARENAPAIR_HOSTED_APP_URL/connect/callback` instead.
     Omitting it breaks headless and SSH authorization.
4. Enable the `openid`, `profile`, and `email` scopes.
5. Set `ARENAPAIR_CLERK_CLI_OAUTH_CLIENT_ID` in the repository-root `.env` file and release build
...
Set `ARENAPAIR_CLERK_JWT_TEMPLATE=t3-relay` in the repository-root `.env`, and set
...
is shared by production and non-production relay stages. The client-facing `ARENAPAIR_RELAY_URL` still
...
Local desktop development uses `arenapair-dev://app`, while packaged builds use `arenapair://app`. Add the
matching origin to each Clerk instance's Backend API `allowed_origins` array as well. The development
Clerk instance should only need `arenapair-dev://app`; the production Clerk instance should only need
`arenapair://app`. `@clerk/electron` owns the native request adapter, encrypted Clerk token persistence,
...
curl -X PATCH https://api.clerk.com/v1/instance \
  ...
  -d '{"allowed_origins":["arenapair://app"]}'
```
...
The production macOS bundle ID is `com.t3tools.arenapair`. To enable native passkeys:

1. Create an explicit macOS App ID for `com.t3tools.arenapair` in the Apple Developer portal and enable
   **Associated Domains**.
...
4. Confirm Clerk serves `https://<frontend-api>/.well-known/apple-app-site-association` and that
   `webcredentials.apps` contains `<TEAM_ID>.com.t3tools.arenapair`.
...
ARENAPAIR_MACOS_PROVISIONING_PROFILE=/absolute/path/to/arenapair.provisionprofile
# Optional: comma-separated override when Clerk's RP ID differs from the Frontend API hostname.
ARENAPAIR_CLERK_PASSKEY_RP_DOMAINS=example.clerk.accounts.dev,clerk.example.com
```

When `ARENAPAIR_CLERK_PASSKEY_RP_DOMAINS` is absent, the build derives the RP domain from
`ARENAPAIR_CLERK_PUBLISHABLE_KEY`. Signed macOS builds fail early if the Team ID, provisioning profile,
...
installed app executable with `VITE_DEV_SERVER_URL` and `ARENAPAIR_PORT` set. Rebuild the signed app
...
ARENAPAIR_PORT=13773 \
  "/Applications/Arena Pair (Alpha).app/Contents/MacOS/Arena Pair (Alpha)"
```
...
```sh
codesign --verify --deep --strict "/Applications/Arena Pair (Alpha).app"
codesign -d --entitlements :- "/Applications/Arena Pair (Alpha).app"
```
