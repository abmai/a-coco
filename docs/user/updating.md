# Keeping Arena Pair in Sync

The Arena Pair web or desktop app and the server it connects to work best when they use the same
version. If they do not match, Arena Pair shows a warning with the right update option for that server.
...
| Action                     | What to do                                                                                                                                                                  |
| -------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Update server**          | Available for the Arena Pair Linux background service. Select the button and leave Arena Pair open while it prepares, tests, restarts, and reconnects.                            |
| **Update the desktop app** | Open the Arena Pair desktop app on the machine that runs the server and install the app update there. Reopen it if needed.                                                     |
| **Copy update command**    | Copy the command, open a terminal on the server machine, stop the current Arena Pair server, and relaunch it with the copied command and any startup options you normally use. |

The available action depends on how that server was started. Arena Pair does not update connected
...
If the server instead runs as the Arena Pair background service, update the service on the host and
...
See [Running Arena Pair in the Background](./background-service.md) for install, status, and removal
commands.
