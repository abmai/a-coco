...
This guide is for people who want to use more than one Codex account in Arena Pair. For Claude, see
[Claude](./providers-claude.md). For first-time setup, see [Install Arena Pair](./install.md).
...
issue. Arena Pair uploads the thread and Codex logs to OpenAI and shows a thread ID that you can copy
...
reports them. If Codex does not report either value, Arena Pair leaves it out instead of using the
parent agent's settings.

## Approve access to other apps

When a Codex tool needs access to an app such as Safari, Arena Pair shows the app name and asks for
...
In Arena Pair Settings, name it something obvious:
...
In Arena Pair Settings, add another Codex provider:
...
Arena Pair shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, Arena Pair treats it
