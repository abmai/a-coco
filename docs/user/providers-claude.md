...
This guide is for people who want to use more than one Claude setup in Arena Pair. For Codex, see
[Codex](./providers-codex.md). For first-time setup, see [Install Arena Pair](./install.md).
...
In Arena Pair Settings, your Claude provider can stay like this:
...
An empty `CLAUDE_CONFIG_DIR path` means Arena Pair uses Claude Code's normal config directory.

When you set this field, Arena Pair points Claude Code at that directory with the
...
On web and desktop, when you return to an older Claude thread with a large context, Arena Pair
...
Arena Pair looks for Claude skills in the Claude config directory's `skills` folder, then
...
`~/.claude_personal_home/.claude`, which is not where Arena Pair looks.

Then add another Claude provider in Arena Pair:
...
Arena Pair only offers Claude providers that use the same config directory for an existing thread. A
...
multiple files under its config directory, so Arena Pair keeps separate config directories isolated
...
Add or edit a Claude provider in Arena Pair Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. Arena Pair stores the value as a server secret and does not
...
Arena Pair does not need a special Claude Code Router provider. Treat the router as a Claude
