...
```sh
vp run --filter arenapair-relay deploy -- --stage "$USER" --env-file .env.local
```
...
- `ARENAPAIR_WEB_ROUTER_URL`: defaults to `https://app.t3.codes`.
- `ARENAPAIR_WEB_LATEST_DOMAIN`: defaults to `latest.app.t3.codes`.
- `ARENAPAIR_WEB_NIGHTLY_DOMAIN`: defaults to `nightly.app.t3.codes`.
...
visiting `/__arenapair/channel?channel=latest` or
`/__arenapair/channel?channel=nightly`; the router stores the
`arenapair_web_channel` cookie and rewrites future requests on `app.t3.codes` to
...
through `/__arenapair/channel` on the router domain so the user's channel cookie is
...
- Repository slug source:
  - `ARENAPAIR_DESKTOP_UPDATE_REPOSITORY` (format `owner/repo`), if set.
...
2. Create an explicit App ID for `com.t3tools.arenapair` and enable Associated Domains.
...
- macOS build unsigned when expected signed:
  ...
  - Confirm the provisioning profile belongs to `APPLE_TEAM_ID.com.t3tools.arenapair` and includes
    Associated Domains.
