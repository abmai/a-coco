...
Arena Pair has one server-side observability model:
...
```bash
export ARENAPAIR_OTLP_TRACES_URL=http://localhost:4318/v1/traces
export ARENAPAIR_OTLP_METRICS_URL=http://localhost:4318/v1/metrics
export ARENAPAIR_OTLP_SERVICE_NAME=t3-local
```

Optional:

```bash
export ARENAPAIR_TRACE_MIN_LEVEL=Info
export ARENAPAIR_TRACE_TIMING_ENABLED=true
```
...
Launch the actual app executable from the same shell so the desktop app and embedded backend inherit `ARENAPAIR_OTLP_*`.

macOS app bundle example:

```bash
ARENAPAIR_OTLP_TRACES_URL=http://localhost:4318/v1/traces \
ARENAPAIR_OTLP_METRICS_URL=http://localhost:4318/v1/metrics \
ARENAPAIR_OTLP_SERVICE_NAME=t3-desktop \
"/Applications/Arena Pair.app/Contents/MacOS/Arena Pair"
```
...
```bash
TRACE_FILE="${ARENAPAIR_HOME:-$HOME/.t3}/userdata/logs/server.trace.ndjson"
```
...
- `ARENAPAIR_OTLP_TRACES_URL` was not set
...
- `ARENAPAIR_TRACE_FILE`: override trace file path
- `ARENAPAIR_TRACE_MAX_BYTES`: per-file rotation size, default `10485760`
- `ARENAPAIR_TRACE_MAX_FILES`: rotated file count, default `10`
- `ARENAPAIR_TRACE_BATCH_WINDOW_MS`: flush window, default `200`
- `ARENAPAIR_TRACE_MIN_LEVEL`: minimum trace level, default `Info`
- `ARENAPAIR_TRACE_TIMING_ENABLED`: enable timing metadata, default `true`

OTLP export:

- `ARENAPAIR_OTLP_TRACES_URL`: OTLP trace endpoint
- `ARENAPAIR_OTLP_METRICS_URL`: OTLP metric endpoint
- `ARENAPAIR_OTLP_EXPORT_INTERVAL_MS`: export interval, default `10000`
- `ARENAPAIR_OTLP_SERVICE_NAME`: service name, default `t3-server`
